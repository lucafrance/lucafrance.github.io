---
title: Serve una traduzione italiana di "dickover"
tags: []
mastodon: 
---

TL;DR: propongo "sovraminichia".

Il 29 maggio 2026 John Gruber [ha proposto](https://daringfireball.net/2026/05/what_is_a_dickover) un neologismo che [ha preso](https://news.ycombinator.com/item?id=48330882) [piede](https://daringfireball.net/linked/2026/08/18/dickover-makes-it-into-the-guardian) [velocemente](https://en.wiktionary.org/wiki/dickover): *dickover*.

> **dickover** n. : a modal panel, popover, or curtain presented by a website or app, deliberately obscuring its own content to frustrate the user with an unwanted, unnecessary, mandatory interaction; e.g. asking the user to accept “cookies”, subscribe to a newsletter, install the website’s mobile app, agree to terms of service, or anything else that the user couldn’t give two shits about.

{:refdef: style="text-align: center;"}
![A cockerel sitting on a tree and crowing](/assets/2026/dickover/junglefowl-on-tree.jpg){: width="70%" }<br>
© Philip Pikart / [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/legalcode) ([via Wikimedia Commons](https://commons.wikimedia.org/wiki/File:Junglefowl_on_tree.jpg))
{: refdef}

La lingua italiana ha bisogno di una parola per esprimere lo stesso concetto.
Usare il termine inglese sarebbe un errore, perché la carica emotiva verrebbe compromessa.
Se *dickover* diventasse comune in italiano, qualcuno lo userebbe seriamente:
"Potremmo mettere un *dickover* per l'iscrizione alla *newsletter*."; "Certo possiamo aggiungerlo agli altri *dickover*".

> You know what a dickover is, even if you didn’t know what to call it (until now). If you use the Internet, you encounter them every day. They’re popovers, but dickheaded. The web is absolutely lousy with them, and mobile apps present them too, with increasing frequency.

Propongo *sovraminchia*, che penso riproduca bene la connotazione inglese.
- **sovra-** *Dickover* deriva da [*popover*](https://developer.apple.com/design/human-interface-guidelines/popovers), di cui non ho trovato traduzioni soddisfacenti in italiano. Ho scelto il prefisso "[sovra](https://www.treccani.it/vocabolario/sovra/?search=s%C3%B3vra-)" per rendere esplicito come si sovrapponga ai contenuti rilevanti del sito.
- **-minchia** Ci sono molte varianti regionali fra cui scegliere. Ho optato per "minchia" pensando a Dario Bressanini, che [ha raccontato](https://youtu.be/IK2NuNMDcws?si=j3iw3iM8062tGVbZ&t=955) di come gli sia stato impedito di chiamare "Acque della minchia" un capitolo del libro "Fa bene o fa male?".
